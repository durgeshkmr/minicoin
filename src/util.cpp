...
const char * const MINICOIN_CONF_FILENAME = "minicoin.conf";
const char * const MINICOIN_PID_FILENAME = "minicoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "minicoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Minicoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Minicoin
    // Mac: ~/Library/Application Support/Minicoin
    // Unix: ~/.minicoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Minicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Minicoin";
#else
    // Unix
    return pathRet / ".minicoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MINICOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", MINICOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Minicoin Core") == std::string::npos) {
