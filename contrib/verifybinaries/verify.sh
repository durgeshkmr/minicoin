...
###   minicoincore.org and minicoin.org and compares them.
...
WORKINGDIR="/tmp/minicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://minicoincore.org"
HOST2="https://minicoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="minicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'minicoin-',
...
if [ $? -ne 0 ]; then
   echo "minicoin.org failed to provide signature file, but minicoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "minicoin.org and minicoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Minicoin Core binary release signing key installed?"
   fi
